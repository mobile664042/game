...
//		if(!gameSessionInfo.getAddress().getAddrNo().startsWith(tableDesk.getAddrNo())) {
//			throw new BizException(String.format("已在%s, 不可以再进入%s", gameSessionInfo.getAddress().getAddrNo(), tableDesk.getAddrNo()));
//		}
//		
...
//			throw new BizException(String.format("%s还未建立连接", player.getId()));
//		}
//		if(gameSessionInfo.getAddress() != null) {
//			logger.info("{}重新进入{}游戏:所在位置{}, 请求游戏桌{}", player.getNickname(), gameItem.getName(), gameSessionInfo.getAddress().getAddrNo(), tableDesk.getAddrNo());
...
////		gameSessionInfo.setPlayerId(player.getId());
//		gameSessionInfo.setAddress(tableDesk);
...
//				int position = ((GameSeat)gameSessionInfo.getAddress()).getPosition();
//				tableDesk.standUp(player, position);
//				logger.info("强制在席位:{},将{}踢走", gameSessionInfo.getAddress(), player.getNickname());
//			}
//		}
//	}
//}
