package com.simple.game.ddz.domain.cmd.req.seat;

import com.simple.game.core.domain.cmd.req.seat.ReqSeatCmd;
import com.simple.game.ddz.domain.cmd.push.seat.PushDoubleShowCardCmd;

import lombok.Data;

@Data
public class ReqDoubledShowCardCmd extends ReqSeatCmd{
	public final static int CMD = 151006;
	@Override
	public int getCmd() {
		return CMD;
	}

	@Override
	public String toLogStr() {
		// TODO Auto-generated method stub
		return null;
	}

	public PushDoubleShowCardCmd valueOfPushShowCardCmd() {
		PushDoubleShowCardCmd pushCmd = new PushDoubleShowCardCmd();
//		pushCmd.setDeskNo(deskNo);
//		pushCmd.setPlayKind(playKind);
//		pushCmd.setPosition(position);
//		pushCmd.setScore(score);
		return pushCmd;
	}
}
